"""QA Tool UI Module - PySide Integration

Author: Thai Ha Trieu (Hannah Trieu)
Studio: Vintata Animation Studio
Role: Rigger

Provides the main user interface for the QA Tool using PySide/PyQt5,
enabling riggers to validate and clean character and prop rigs before
export to animation.
"""
...
Setup Instructions:
import sys
sys.path.append("C:/Users/YOUR_NAME/Documents/maya/2017/scripts/QATool/")
...
import re
import os
import matrixTools as matrix
from matrixTools import Ui_matrixWindow
...
class QAtool_UI(QtWidgets.QWidget):
    """Main UI class for QA Tool.
    
    Provides a comprehensive interface for character and prop rig validation
    and cleanup before export to animation department.
    """
    ...
    def __init__(self):
        ...
        super(QAtool_UI, self).__init__(parent=parent)
    ...
    def buildUI(self):
        ...
        self.maCheckBox.clicked.connect(self.maClicked)

        self.mbCheckBox = QtWidgets.QRadioButton(".mb")
        hLayout.addWidget(self.mbCheckBox)
        self.mbCheckBox.clicked.connect(self.mbClicked)
...
#--		
	...
	def mbClicked(self):
		curentText = self.expDir.text()
		findIt = re.search(r"[^\.]*$", curentText).group()
		curentText = curentText.replace(("."+findIt), ".mb")
		self.expDir.setText(curentText)

	def maClicked(self):
		curentText = self.expDir.text()
		findIt = re.search(r"[^\.]*$", curentText).group()
		curentText = curentText.replace(("."+findIt), ".ma")
		self.expDir.setText(curentText)
...
#--------------------------------
	def autoDir(self): #done
		...
		if not os.path.exists(self.longSceneName):	
			...
			findIt = re.search(r"[^\.]*$", self.longSceneName).group()
			curentText = self.longSceneName.replace(("."+findIt), ("."+typExp))
			final = curentText 
		else:
...
# copy ------------------------------------------
	...
	def makeAnimModeN(self): #done
		...
		if not self.rigGrpName:
			...
			for x in jnts:
				...
				if test:
					eCtrl = [i for i in test if "ctrl" in str(i)]
					if eCtrl:
						mel.eval("catchQuiet (`setAttr  "+ x + ".drawStyle 2`)")
	...
	def convertMatrixNlock(self): 
		exec(matrix.interGrated_QATools)	
